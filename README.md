...
**Tests:** ~7,200 backend tests across ~200 modules (incl. the mutation-verified `security/` suite **and a black-box live-API pentest suite** — auth/IDOR/scope/injection/RLS/SSRF/DoS/secret attacks over a seeded DB) + conformance suite (golden Arrow output + byte-identical cache keys), live API E2E, MCP tests, CLI tests, ~930 dashboard + ~360 embed-component tests (`node --test` / vitest), plus browser embed E2E (Playwright). All green on `main`.
