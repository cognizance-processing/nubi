...
- **Flows as a data-app engine** — per-cell `cpu_cores / mem_mb / timeout_s` resource requests; stochastic cells with run-level seeds for reproducibility; typed artifact channel (pickle/joblib/model blobs by handle); scenario sweep + backfill; event/webhook/downstream triggers + run-history + SLA monitoring; governed write-back (idempotent/transactional, dry-run, RBAC, approval gates). An SSRF-guarded `http_call` task kind lets a flow call out to a host endpoint; an `assert` task kind runs data-quality expectations (`row_count`/`not_null`/`unique`/`custom_sql`) and fails the run on violation — the flows equivalent of a SQLMesh audit. Powers the close-the-loop cycle: Flow computes → table → metric → dashboard → action → write-back → re-trigger. See [docs/flows.md](docs/flows.md).
- **Canvas — HTML-native document surface** — a `CanvasDoc` is free-form HTML authored directly (by a human or LLM). Elements bind to queries, semantic metrics, or HTTP API connectors via a side `data-el-id` → binding map. Reuses the dashboards data/RLS/sanitizer/scheduling layer; adds a code+visual editor, a right-hand binding inspector, a `/c/:id` public viewer, and a generate→validate→repair LLM loop. Scheduled sends (PDF/HTML) reuse the `report_send` flow handler.
...
- **Inter-model lineage DAG, incl. column-level** — `GET /lineage/dag` builds a directed graph across all queries, metrics, and physical tables; `GET /lineage/dag/{node_id}?hops=N` walks N hops upstream/downstream. `resolve_column_lineage` traces a single column's provenance across model layers — alias-aware, cycle-safe, with a `SELECT *` fallback. Metric lineage (`GET /metrics/{id}/lineage`) resolves input columns and upstream tables; flow/notebook cell lineage traces column provenance across cells. An opt-in `auto_rebuild_downstream` flow flag fires dependent flows on upstream success (DAG traversal, cycle- and storm-safe). See [docs/lineage.md](docs/lineage.md).
...
- **Data-custody tier for regulated data** — an opt-in tier giving hosts BYO storage, customer-managed encryption keys, and **region pinning** for POPIA/GDPR-relevant residency requirements, plus a versioned write/ingest API (land Parquet → atomic publish, idempotent full-replace + incremental append, schema declare/evolve). See [docs/custody-tier.md](docs/custody-tier.md) and the honest gap disclosure in [docs/compliance.md](docs/compliance.md).
- **Comprehensive chart library** — `<nubi-chart>` renders **17 types** (bar grouped/stacked/100%/horizontal, line, area, scatter, bubble, pie, donut, **sankey**, funnel, **waterfall**, heatmap, radar, treemap, boxplot, gauge, candlestick, and a **forecast fan chart**) with a full `config` (legend, palette, stacking, data labels, log/dual-Y axes, currency/percent/SI/date formatting, reference/target lines, annotations), theme-driven light/dark. It's a framework-agnostic web component embedding hosts drop into a panel, and the same builder powers Nubi's own dashboards. `<nubi-table>`/`<nubi-metric-explorer>` ship client-side **CSV export**.
...
| Flows data-app engine (Axis B) | ✅ Done | Per-cell `cpu_cores / mem_mb / timeout_s`; stochastic cells + run-level seeds; typed artifact channel (pickle/joblib/bytes/json); scenario sweep + backfill; event/webhook/downstream triggers + run-history + SLA; governed write-back (idempotent, dry-run, RBAC, approval gates) |
| Canvas (Axis C) | ✅ Done | `canvases` resource; `CanvasDoc` model; `validate_canvas_doc`; `/c/:id` public viewer; code+visual editor + RHS binding inspector; three binding kinds (query/metric/api); LLM generate/edit/repair loop; scheduled send via `report_send` |
...
- [Lineage](docs/lineage.md) — dependency DAG + column-level lineage + auto-rebuild · [Transformation](docs/transformation.md) — versioning/revert, backfill, transpile
