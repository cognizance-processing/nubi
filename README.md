...
| Dashboard editor | ✅ Done | Single authoring surface — the dashboard grid (`DashboardEditor`); live data-bound widgets, drag/resize layout, shared inspector/palette. One board is the source of truth for every output (embed, PDF, scheduled email) |
...
- [MCP integration](docs/mcp.md) — host server registry, agent dispatch, Nubi-as-MCP-server · [Embed API](docs/embed-api.md) — the 9-component contract
