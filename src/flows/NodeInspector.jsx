  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        {snippetOpen && (
          <div className="absolute z-20 top-full left-0 mt-1 min-w-[220px] p-1.5 rounded-xl bg-surface border border-border shadow-lg shadow-black/10">
    ...
    <aside className="flex flex-col h-full border-l border-border bg-surface overflow-hidden">
      ...
      <fieldset disabled={readOnly} className="flex-1 overflow-y-auto px-4 py-4 space-y-5 min-w-0 disabled:opacity-70">
        ...
        <section className="space-y-3">
          ...
          {(() => {
            // Insert a `{{ vars.NAME }}` token into the cell's text field. SQL →
            // config.sql, python → config.code, agent → config.prompt; other
            // kinds have no text target, so insertion is hidden.
            const field = task.kind === 'query' ? 'sql'
              : task.kind === 'python' ? 'code'
              : task.kind === 'agent' ? 'prompt'
              : null
            const onInsert = field && !readOnly
              ? (name) => {
              ...
              : null
            return <VariablesPanel onInsert={onInsert} readOnly={readOnly} />
          })()}
        </section>
