...
import {
    ...
    ArrowRight, Sparkles, Send, Clock, BarChart3,
} from 'lucide-react'

export default function HomePage() {
    const navigate = useNavigate()
    const { user } = useAuth()
    const { setChatOpen, openChatFor } = useChat()

    const [boards, setBoards] = useState([])
    const [loading, setLoading] = useState(true)
    const [stats, setStats] = useState({ boards: 0, queries: 0, datastores: 0, chats: 0 })
    ...
    const fetchData = async () => {
        try {
            const [boardsRes, queriesRes, datastoresRes, chatsRes] = await Promise.all([
                supabase.from('boards').select('*').order('created_at', { ascending: false }).limit(6),
                supabase.from('board_queries').select('id', { count: 'exact', head: true }),
                supabase.from('datastores').select('id', { count: 'exact', head: true }),
                supabase.from('chats').select('id', { count: 'exact', head: true }),
            ])

            setBoards(boardsRes.data || [])
            setStats({
                boards: boardsRes.count ?? boardsRes.data?.length ?? 0,
                queries: queriesRes.count || 0,
                datastores: datastoresRes.count || 0,
    ...
    const statCards = [
        ...
        { label: 'Chats', value: stats.chats, icon: MessageSquare, color: 'text-amber-400 bg-amber-500/10 border-amber-500/15' },
    ]
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                            {boards.map((board) => (
                                <button
                                    ...
                                    className="group text-left rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 hover:border-white/[0.12] hover:bg-white/[0.04] transition-all"
                                >
                                    ...
                                    {board.description && (
                                        <p className="text-xs text-slate-500 leading-relaxed line-clamp-2 mb-3">{board.description}</p>
                                    )}
