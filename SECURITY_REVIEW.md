...
## Residual Risk (all prior items now fixed or closed)

1. **DNS rebinding on delivery** (FIXED — commit `security/close-residuals`): `deliver_one` now calls `resolve_and_pin` (from `app.connectors.ssrf`) after the `guard_url` pre-flight check. The `_build_pinned_client` helper builds a custom `_PinnedTransport` (httpcore `AsyncConnectionPool` targeting the pinned IP) that bypasses further DNS resolution. TLS SNI and `Host` header are preserved so cert validation still works. A secondary fail-closed check inside the transport rejects any private IP that somehow slips through. Zero-TTL rebinding is fully closed on the delivery path.

2. **Host-mode write escalation via issuer scope policy** (FIXED — commit `security/close-residuals`): `_maybe_pin_host_mode_org` in `app/auth/deps.py` now strips `write:*`, `edit:*`, `admin`, `admin:*`, and `author:sql` from `identity.scope` immediately after pinning the org, before returning to the caller. The stripping is performed by `_strip_host_mode_write_scopes` which is ONLY called on the host-mode code path; non-host-mode tokens are completely unaffected. `author:metric` is kept (it is a legitimate embed authoring scope). Covered by 16 new tests in `tests/security/test_sec_host_mode_scope_strip.py`.

3. **SSRF via webhook URL field in `schemas.py`** (informational, unchanged): The `WebhookCreate.url` field is typed as `str` with no format validation beyond Pydantic's basic string handling. `guard_url` provides scheme validation. Not changed because adding `HttpUrl` would change serialization behavior and `guard_url` catches the relevant attacks.
