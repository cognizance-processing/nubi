...
# ---------------------------------------------------------------------------
# POST /ai/canvas — generate a CanvasDoc from a question (Canvas Wave 6)
# ---------------------------------------------------------------------------


class CanvasGenerateRequest(BaseModel):
    """Request body for POST /ai/canvas."""
...
class CanvasGenerateResponse(BaseModel):
    """Response body for POST /ai/canvas."""
...
async def generate_canvas(
    ...
    """Generate a grounded CanvasDoc for *question* (Canvas Wave 6).

    Pipeline
    --------
    1. Build the catalog from the live query registry + lineage graph.
    2. Run deterministic grounding to find relevant tables/columns/queries.
    3. Call ``generate_canvas_doc`` — with NullProvider (the default) this is a
       pure in-memory operation.  With a real provider the LLM generates a JSON
       CanvasDoc through the validate -> repair loop.
    4. Run ``validate_canvas_doc`` as a server-side sanity check.
    5. Return the doc dict, HTML, grounding context, provider name, and
       validation result.

    Returns
    -------
    CanvasGenerateResponse
        ``{doc, html, grounding, provider, valid, issues}``
    ...
    AppError("canvas_generation_failed", 422)
        Real-provider path: when the LLM output still fails after max repair rounds.
    """
    from app.ai.canvas import generate_canvas_doc  # noqa: PLC0415
    from app.dashboards.canvas import validate_canvas_doc  # noqa: PLC0415
    ...
    await _record_ai_call(_user, org_id, endpoint="ai_canvas")
...
# ---------------------------------------------------------------------------
# POST /ai/canvas/edit — apply a natural-language edit to a CanvasDoc
# ---------------------------------------------------------------------------


class CanvasEditRequest(BaseModel):
    """Request body for POST /ai/canvas/edit."""
...
class CanvasEditResponse(BaseModel):
    """Response body for POST /ai/canvas/edit."""
...
async def edit_canvas(
    ...
    Pipeline
    --------
    1. Parse the incoming ``doc`` dict into a CanvasDoc.
    2. Build the catalog and run grounding on the instruction.
    3. Call ``edit_canvas_doc`` — same validate/repair loop as generation.
    4. Return the updated doc, its HTML, grounding context, provider, and
       validation result.
    ...
    AppError("canvas_edit_failed", 422)
        Real-provider path: when the LLM output still fails after max repair rounds.
    """
    from app.ai.canvas import edit_canvas_doc  # noqa: PLC0415
    from app.dashboards.canvas import CanvasDoc, validate_canvas_doc  # noqa: PLC0415

    org_id = await _enforce_ai_quota(_user)

    # Parse the incoming doc dict into a CanvasDoc model.
    try:
        canvas_doc = CanvasDoc(**body.doc)
    except Exception as exc:  # noqa: BLE001
        raise AppError("invalid_canvas_doc", f"Could not parse doc: {exc}", 400) from exc
    ...
    grounding = ground(body.instruction, catalog)

    await _record_ai_call(_user, org_id, endpoint="ai_canvas_edit")

    return CanvasEditResponse(
        doc=updated_doc.model_dump(),
...
# ---------------------------------------------------------------------------
# GET /ai/canvas/schema — JSON Schema for CanvasDoc
# ---------------------------------------------------------------------------


@api_router.get("/ai/canvas/schema", tags=["ai", "canvas"])
async def canvas_schema(
    _user: dict[str, Any] = Depends(current_user),
) -> dict[str, Any]:
    """Return the JSON Schema for the canonical CanvasDoc format.

    Exposes the full CanvasDoc schema (including all binding kinds) so that:
    - The frontend canvas editor knows the exact format to read/write.
    - The LLM authoring pipeline can be grounded with the schema.
    - External tools (MCP clients) can validate docs before submitting.

    Returns
    -------
    dict
        JSON Schema dict with ``canvas_doc`` (Pydantic v2 output) and
        ``binding_kinds`` listing the supported binding kind identifiers.
    ...
    AppError("unauthorized", 401)
        If no valid Bearer token is provided.
    """
    from app.ai.canvas import canvas_doc_json_schema  # noqa: PLC0415

    return {
        "canvas_doc": canvas_doc_json_schema(),
        "binding_kinds": ["query", "metric", "api"],
        "allowed_widget_tags": sorted(
            [
                "nubi-kpi",
                "nubi-table",
                "nubi-chart",
