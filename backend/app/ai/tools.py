...
def _tool_explain_metric_change(
    claims: dict[str, Any],
    metric_id: str,
    current_start: str,
    current_end: str,
    ...
    """Explain what drove a metric change between two time windows.

    Runs the SAME computation as ``POST /metrics/{id}/explain`` — fetches the
    metric grouped by each allowed dimension for both the *current* and
    *comparison* periods, computes per-member delta contributions via
    ``build_explain_result``, and returns a ranked breakdown the model can
    verbalize.

    Parameters
    ----------
    claims:
        Caller's auth claims (RLS enforced — data never exceeds caller scope;
        tenant isolation gate identical to query_metric).
    metric_id:
        Id of a governed metric (must be in the registry and belong to the
        caller's org, or be a built-in seed metric).
    current_start / current_end:
        ISO datetime strings for the current period ``[start, end)``.
    comparison_start / comparison_end:
        ISO datetime strings for the comparison period ``[start, end)``.
    dimensions:
        Optional subset of the metric's declared dimensions to analyse.
        ``None`` → all allowed dimensions.
    top_n:
        Maximum members per dimension before collapsing to "Other" (1–50).
    ...
    dict
        On success::

            {
                "metric_id": str,
                "measure": str,
                "delta_total": float,       # current_total - comparison_total
                "current_total": float,
                "comparison_total": float,
                "dimensions": [             # sorted by explanatory_power desc
                    {
                        "dimension": str,
                        "explanatory_power": float,
                        "coverage": float,
                        "members": [        # top_n members by |delta|
                            {
                                "member": any,
                                "current": float | null,
                                "comparison": float | null,
                                "delta": float,
                                "share": float,   # delta / |delta_total|
                                "direction": "up"|"down"|"flat"
                            }, ...
                        ],
                        "other": {...} | null   # aggregated tail
                    }, ...
                ],
                "summary_hint": str         # NL hint for the model to verbalize
            }
    ...
    # ── TENANT ISOLATION: same gate as _tool_query_metric ───────────────────
    org_id = claims.get("org")
    if metric_id not in SEED_METRIC_IDS:
        if not org_id or not _run_sync(metric_belongs_to_org(metric_id, str(org_id))):
            return {
    ...
    # ── Determine dimensions to analyse ─────────────────────────────────────
    allowed_dim_names = [d.name for d in metric.dimensions]
    if dimensions is not None:
        unknown = [d for d in dimensions if d not in allowed_dim_names]
        if unknown:
            return {
                "error": {
                    "code": "unknown_dimension",
                    "message": f"Dimension(s) not declared on metric: {unknown}",
                }
            }
        analyze_dims = [d for d in dimensions if d in allowed_dim_names]
    else:
        analyze_dims = allowed_dim_names

    # ── Determine time column (may be None for metrics without a time dim) ──
    td = metric.time_dimension
    time_col: str | None = td.column if td is not None else None

    # ── Policy columns (for RLS-aware compilation) ───────────────────────────
    policy_cols = tuple((claims.get("policies") or {}).keys())

    # ── Inner helper: run one MetricQuery via DuckDB (sync, like query_metric) ─
    def _run_mq(mq_dims: list[str], period_filters: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Compile + plan + execute one MetricQuery; return row dicts."""
        try:
            mq = MetricQuery.from_dict(
                {
                    "metric_id": metric_id,
            ...
            # Governance rejection → empty result (not a hard error; analysed dims
            # that fail compilation are simply skipped / treated as zero).
            return []
    ...
    # ── Build time-range filters for each period ─────────────────────────────
    def _period_filters(start: str, end: str) -> list[dict[str, Any]]:
        if time_col is None:
            return []
        return [
            {"field": time_col, "op": ">=", "value": start},
            {"field": time_col, "op": "<", "value": end},
        ]

    cur_filters = _period_filters(current_start, current_end)
    cmp_filters = _period_filters(comparison_start, comparison_end)

    # ── Fetch totals (no groupby) ─────────────────────────────────────────────
    measure_name = metric.measure.name

    def _extract_total(rows: list[dict[str, Any]]) -> float:
        if not rows:
            return 0.0
    ...
    cur_total_rows = _run_mq([], cur_filters)
    cmp_total_rows = _run_mq([], cmp_filters)
    current_total = _extract_total(cur_total_rows)
    comparison_total = _extract_total(cmp_total_rows)

    # ── Fetch per-dimension breakdowns ────────────────────────────────────────
    current_aggs: dict[str, dict[Any, float]] = {}
    comparison_aggs: dict[str, dict[Any, float]] = {}

    for dim_name in analyze_dims:
        for period_label, filters in (("current", cur_filters), ("comparison", cmp_filters)):
            rows = _run_mq([dim_name], filters)
            agg: dict[Any, float] = {}
            for row in rows:
    ...
    # ── Build dimension_breakdowns dict and call pure math ────────────────────
    dimension_breakdowns = {
        dim_name: (
            current_aggs.get(dim_name, {}),
            comparison_aggs.get(dim_name, {}),
        )
        for dim_name in analyze_dims
    }
    ...
    # ── Build NL summary hint ─────────────────────────────────────────────────
    delta = explain_result.delta_total
    direction_word = "increased" if delta > 1e-9 else ("decreased" if delta < -1e-9 else "unchanged")
    top_dim_hints: list[str] = []
    for bd in explain_result.dimensions[:3]:
        top_member = bd.members[0].member if bd.members else None
        if top_member is not None:
            top_dim_hints.append(f"{bd.dimension}={top_member!r}")
    hint_parts = [
        f"Metric '{metric_id}' ({measure_name}) {direction_word} by {abs(delta):.4g} "
        f"(current={current_total:.4g}, comparison={comparison_total:.4g})."
    ]
    if top_dim_hints:
        hint_parts.append(f"Top drivers: {', '.join(top_dim_hints)}.")

    summary_hint = " ".join(hint_parts)

    # ── Serialise to JSON-safe dict (mirrors REST endpoint shape) ─────────────
    def _mc_to_dict(mc: Any) -> dict[str, Any]:
        return {
            "member": mc.member,
            "current": mc.current,
        ...
        dims_out.append(
            {
                ...
                "members": [_mc_to_dict(m) for m in bd.members],
                "other": _mc_to_dict(bd.other) if bd.other else None,
            }
        )
...
_SCHEMA_EXPLAIN_METRIC_CHANGE: dict[str, Any] = {
    "type": "object",
    "properties": {
        "metric_id": {
            "type": "string",
        ...
        "current_start": {
            "type": "string",
            "description": "ISO datetime string — inclusive start of the current period.",
        },
        "current_end": {
            "type": "string",
            "description": "ISO datetime string — exclusive end of the current period.",
        },
        "comparison_start": {
            "type": "string",
            "description": "ISO datetime string — inclusive start of the comparison period.",
        },
        "comparison_end": {
            "type": "string",
            "description": "ISO datetime string — exclusive end of the comparison period.",
        },
        "dimensions": {
            "type": "array",
            "items": {"type": "string"},
            "description": (
                "Optional subset of the metric's declared dimensions to analyse. "
                "Omit to analyse all dimensions."
            ),
        },
        "top_n": {
            "type": "integer",
            "description": "Max members per dimension before collapsing to 'Other' (1–50, default 10).",
        },
    },
    "required": ["metric_id", "current_start", "current_end", "comparison_start", "comparison_end"],
    "additionalProperties": False,
}
...
def _make_registry() -> dict[str, ToolDef]:
    ...
    def _wrap_explain_metric_change(
        claims: dict[str, Any],
        metric_id: str,
        current_start: str,
        current_end: str,
        ...
        **_kw: Any,
        ...
        return _tool_explain_metric_change(
            claims,
            metric_id,
    ...
    tools = [
        ...
        ToolDef(
            name="explain_metric_change",
            description=(
                "Explain what drove a metric change between two time windows (current vs comparison). "
                "Returns a ranked per-dimension contribution breakdown: delta_total, "
                "current/comparison totals, top members with delta + share + direction, "
                "and a summary_hint for verbalization. Use this to answer 'why did X change / "
                "what drove the movement in metric Y' questions. "
                "Caller's RLS claims are enforced — data never exceeds caller scope."
            ),
            json_schema=_SCHEMA_EXPLAIN_METRIC_CHANGE,
            fn=_wrap_explain_metric_change,
        ),
        ToolDef(
