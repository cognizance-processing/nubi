...
the SINGLE source for all demo data paths:

- **Managed lakehouse configured** (S3 in cloud OR local-file backend in dev —
  ``NUBI_BUCKET_URI``/``S3_*`` or ``NUBI_MANAGED_LAKE_DIR``): per-project
  parquet files under
  ``<lake-root>/orgs/<org>/projects/<project>/demo/<dataset>/<table>.parquet``
  (``app.demo_lakehouse.provision_demo_parquet`` + ``editable_demo_datastore_config``).
  Tables carry ``_row_id`` and are EDITABLE via rewrite-on-edit.

- **No managed lakehouse** (offline dev / CI): parquet is written once to the
  deterministic local directory ``backend/seed_data/parquet/<dataset>/<table>.parquet``
  (``export_demo_parquet_local`` + ``local_parquet_datastore_config``) — same
  parquet + ``read_parquet`` views path, read-only.
...
offline source; the lakehouse object-storage files are the cloud source.
